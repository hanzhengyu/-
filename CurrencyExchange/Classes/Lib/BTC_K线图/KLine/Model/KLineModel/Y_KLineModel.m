...
- (void) initWithDict:(NSDictionary *)dict
{
    ...
    if (self)
    {
        NSLog(@"===%@",dict);
        _Date = dict[@"date"];
        _Open = @([dict[@"sta"] floatValue]);
        _High = @([dict[@"max"] floatValue]);
        _Low = @([dict[@"mix"] floatValue]);
        _Close = @([dict[@"end"] floatValue]);
        _Volume = [dict[@"num"] floatValue];
        self.SumOfLastClose = @(_Close.floatValue + self.PreviousKlineModel.SumOfLastClose.floatValue);
        self.SumOfLastVolume = @(_Volume + self.PreviousKlineModel.SumOfLastVolume.floatValue);       //        NSLog(@"%@======%@======%@------%@",_Close,self.MA7,self.MA30,_SumOfLastClose);
